...
> A build plugin that removes `data-*` test attributes from production while keeping them in dev and test — stable E2E selectors, clean production DOM. Ships as `@datapower/unplugin` & `@datapower/bun` across Vite, Rollup, Webpack, esbuild and Bun. Running in production across 4 different Xefi business units.
>
